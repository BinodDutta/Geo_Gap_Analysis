...
try:
    # A. Load Statistical Gap Analysis
...
# 100% = The highest stress observed
...
# 3. Select User-Friendly Columns (Z-Score & Raw Load Removed)
csv_output = top_200[['pincode', 'district', 'state', 'severity', 'Criticality_Index']]

# 4. Save to Folder
csv_path = os.path.join(output_folder, 'Top_200_Critical_Pincodes.csv')
...
plt.title('Severity Distribution of Digital Gaps', fontsize=16, fontweight='bold')
plt.xlabel('Severity Zone')
plt.ylabel('Number of Pincodes')
plt.tight_layout()
plt.savefig(os.path.join(output_folder, 'Visual_1_Severity.png'), dpi=300)

# --- VISUAL 2: TOP 10 STATES ---
print("Generating Visual 2...")
...
if not high_impact.empty:
    ...
    plt.title('Top 10 States with Critical Infrastructure Gaps', fontsize=16, fontweight='bold')
    plt.xlabel('Number of Critical Zones')
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'Visual_2_State_Impact.png'), dpi=300)

# --- VISUAL 3: TOP 15 DISTRICTS ---
print("Generating Visual 3...")
# Note: We use 'raw_load' here just for aggregation, but don't show it in the final CSV
district_group = df_merged.groupby(['district', 'state']).agg({
    'raw_load': 'sum',
    'Criticality_Index': 'mean', # Average Criticality
    'pincode': 'count'
}).reset_index()

district_group.columns = ['District', 'State', 'Total_Gap_Load', 'Avg_Criticality', 'Critical_Pincodes_Count']
top_districts = district_group.sort_values(by='Total_Gap_Load', ascending=False).head(15)

plt.figure(figsize=(12, 8))
sns.barplot(x='Total_Gap_Load', y='District', data=top_districts, palette="magma")
plt.title('Top 15 Districts with Highest Service Demand', fontsize=16, fontweight='bold')
plt.xlabel('Cumulative Demand Volume')
plt.ylabel('District')
plt.tight_layout()
plt.savefig(os.path.join(output_folder, 'Visual_3_Top_Districts.png'), dpi=300)
